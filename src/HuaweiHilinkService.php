 ...
 */
class HuaweiHilinkService
{
    private $client = null; //Our custom HTTP provider.
    private $routerAddress = ''; //This is the one for the router I got.
    ...
    public function generalizedGet($route)
    {
        ...
        $xml = $this->client->get($this->getUrl($route));
    ...
    public function setLedOn($on = false)
    {
        ...
        $xml = $this->client->postXml($this->getUrl('api/led/circle-switch'), $ledXml);
        $obj = new \SimpleXMLElement($xml);
    ...
    public function getSMSBox($boxType = 1, $page = 1, $count = 20, $unreadPreferred = false)
    {
		...
		';
        $xml = $this->client->postXml($this->getUrl('api/sms/sms-list'), $inboxXml);
        $obj = new \SimpleXMLElement($xml);
        return $obj;
    }
    ...
    public function deleteSms($index)
    {
		...
		';
        $xml = $this->client->postXml($this->getUrl('api/sms/delete-sms'), $deleteXml);
        $obj = new \SimpleXMLElement($xml);
        //Simple check if login is OK.
        return ((string)$obj == 'OK');
    }
    ...
    public function sendSms($receiver, $message)
    {
		...
		*/
		...
		';
        $xml = $this->client->postXml($this->getUrl('api/sms/send-sms'), $sendSmsXml);
        $obj = new \SimpleXMLElement($xml);
        //Simple check if login is OK.
        if ((string)$obj != 'OK') {
            return false;
    ...
    public function login($username, $password)
    {
		...
		*/
		...
		<Password>' . base64_encode(hash('sha256', $username . base64_encode(hash('sha256', $password, false)) . $this->client->getToken(), false)) . '</Password>
		</request>
		';
        $xml = $this->client->postXml($this->getUrl('api/user/login'), $loginXml);
        $obj = new \SimpleXMLElement($xml);
        //Simple check if login is OK.
        return ((string)$obj == 'OK');
    }
    ...
    private function prepare()
    {
        ...
        if (strlen($this->sessionInfo) == 0 || strlen($this->tokenInfo) == 0) {
            ...
            $xml = $this->client->get($this->getUrl('api/webserver/SesTokInfo'));
            $obj = new \SimpleXMLElement($xml);
            ...
            $this->client->setSecurity($obj->SesInfo, $obj->TokInfo);
        }
    }
}
